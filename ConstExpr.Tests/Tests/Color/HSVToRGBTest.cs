	...
	[
		...
		Create((h, _, v) =>
		{
			...
			switch (i)
			{
				...
				{
					r = v;
					g = v * Double.MultiplyAddEstimate(f - 1D, 0.5, 1D);
					b = v * 0.5;
				...
				case 2:
				{
					r = v * 0.5;
					g = v;
					b = v * Double.MultiplyAddEstimate(f - 1D, 0.5, 1D);
				...
				case 4:
				{
					r = v * Double.MultiplyAddEstimate(f - 1D, 0.5, 1D);
					g = v * 0.5;
					b = v;
		...
		Create((_, s, _) => ((byte) (s == 0D ? 127.5D : (1D - s) * 127.5D), 127, 127), [ 180, Unknown, 0.5 ])
	];
}