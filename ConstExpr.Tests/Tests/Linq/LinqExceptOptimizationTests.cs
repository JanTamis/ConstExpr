...
public class LinqExceptOptimizationTests : BaseTest<Func<int[], int>>
{
	...
	[
		Create("return (Count_wX25Rw(x) << 1) + Count_vFVZUg(x) * 3 + Count_IyhE7Q(x) + Count_lIg1kw(x) + Count_4oc4tg(x) + Count_87tGZw(x) + Unsafe.BitCast<bool, byte>(Any_Gk_CFg(x)) + Count_uLsNyg(x);"),
