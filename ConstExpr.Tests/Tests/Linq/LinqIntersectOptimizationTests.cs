...
public class LinqIntersectOptimizationTests : BaseTest<Func<int[], int>>
{
	...
	[
		Create("return (Count_AEkyLw(x) << 1) + (Count_FQoOgw(x) << 1) + Count_w6J_9Q(x) + Count_VBWycg(x) + Count_utUoqA(x) + Count_w7iHXw(x) + Count_MK3tdQ(x) + (Contains_FVnsrQ(x) ? 1 : 0) + Count_N_W_CA(x) + Count_0S7iQA(x);"),
