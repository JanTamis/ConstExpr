...
public class LinqAnyOptimizationListTests() : BaseTest<Func<List<int>, int>>(FastMathFlags.AssociativeMath)
{
	...
	[
		Create("return (x.Count > 0 ? 3 : 0) + (Any_pfIHsA(CollectionsMarshal.AsSpan(x)) ? 1 : 0) + (Any_7KBy_w(CollectionsMarshal.AsSpan(x)) ? 1 : 0) + (Contains_rph_Xw(CollectionsMarshal.AsSpan(x)) ? 1 : 0);", Unknown),
