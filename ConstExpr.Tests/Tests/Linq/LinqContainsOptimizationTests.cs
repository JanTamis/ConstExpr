...
public class LinqContainsOptimizationListTests() : BaseTest<Func<List<int>, int>>(FastMathFlags.AssociativeMath)
{
	...
	public override IEnumerable<KeyValuePair<string?, object?[]>> TestCases =>
	[
		Create("return (Contains_vtuwAg(x) ? 5 : 0) + (x.Exists(v => v == 3) ? 1 : 0) + (Contains_OkH2OQ(x) ? 1 : 0);", Unknown),
...
public class LinqContainsOptimizationStringTests() : BaseTest<Func<string[], int>>(FastMathFlags.AssociativeMath)
{
	...
	public override IEnumerable<KeyValuePair<string?, object?[]>> TestCases =>
	[
		Create("return (Array.IndexOf(x, \"hello\") >= 0 ? 1 : 0) + (Array.IndexOf(x, \"world\") >= 0 ? 1 : 0) + (Array.Exists(x, v => String.Equals(v, \"HELLO\", StringComparison.CurrentCultureIgnoreCase)) ? 1 : 0) + (Array.Exists(x, v => v.Length > 3 && v == \"hello\") ? 1 : 0);", Unknown),
...
public class LinqContainsOptimizationComplexTests() : BaseTest<Func<int[], int>>(FastMathFlags.AssociativeMath)
{
	...
	public override IEnumerable<KeyValuePair<string?, object?[]>> TestCases =>
	[
		Create("return (Array.IndexOf(x, 5) >= 0 ? 2 : 0) + (Array.IndexOf(x, 5) >= 0 || Array.IndexOf(x, 15) >= 0 ? 1 : 0) + (Array.Exists(x, v => Int32.IsEvenInteger(v) && v == 4) ? 1 : 0);", Unknown),
...
public class LinqContainsLiteralCollectionTests : BaseTest<Func<int[], int, bool>>
{
	...
	[
		...
		Create("return (uint)(x - 1) <= 2U;", new int[] { 1, 2, 3 }, Unknown),
		Create("return (uint)(x - 1) <= 7U && (0x91u >> x - 1 & 1) != 0;", new int[] { 1, 5, 8 }, Unknown),
		Create("return (uint)(x - 1) <= 4U && (0x15u >> x - 1 & 1) != 0;", new int[] { 1, 3, 5 }, Unknown),
	];
}
