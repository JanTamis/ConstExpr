...
namespace ConstExpr.SourceGenerator.Sample.Tests
{
	internal class DataValidationTests
	{
		public static void RunTests(double varDouble, string varString, int varInt)
		{
			...
			// DetectOutliers - alleen constanten
			Console.WriteLine($"[CONST] DetectOutliers([1, 2, 3, 4, 5], 2): {String.Join(", ", DataValidationOperations.DetectOutliers([ 1, 2, 3, 4, 5 ]))}");

			// DetectOutliers - mixed
			Console.WriteLine($"[MIXED] DetectOutliers([varInt, 2, 3, 4, 5], 2): {String.Join(", ", DataValidationOperations.DetectOutliers([ varInt, 2, 3, 4, 5 ]))}");
