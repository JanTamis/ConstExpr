...
public partial class ConstExprPartialRewriter
{
	...
	private SyntaxNode? TryUnrollForEachLoop(ForEachStatementSyntax node, IReadOnlyList<CSharpSyntaxNode> items, string? breakLabel = null, bool unrollContinueGoto = false)
	{
		...
		// Synthetic foreach nodes (generated by LinqUnroller) are not part of the original
		// source tree, so GetOperation would throw instead of returning null.
		if (node.SyntaxTree == semanticModel.SyntaxTree && semanticModel.GetOperation(node) is IForEachLoopOperation operation)
		{
	...
	public override SyntaxNode? VisitReturnStatement(ReturnStatementSyntax node)
	{
		...
		// A fold can land on a literal of the return expression's own natural type (e.g. `text[0]`
		// folds to the char '|') while an implicit numeric conversion in the original source widens
		// it to the method's actual return type (`int`, here). The real delegate's boxed result -
		// what CreateFolded compares against - reflects that conversion, so the emitted literal has
		// to as well, or a behaviorally-identical fold (`return '|';` and `return 124;` both return
		// 124) reads as a mismatch.
		if (visitedExpression is LiteralExpressionSyntax { Token.Value: { } literalValue }
		    && node.Expression is { } originalExpression
		    && TryCoerceLiteralToConvertedType(literalValue, originalExpression, out var coercedLiteral))
		{
			visitedExpression = coercedLiteral;
		}
	...
	/// <summary>
	///   Re-coerces a folded literal's value to the numeric/char type an implicit conversion in the
	///   original source would have produced at <paramref name="originalExpression" />'s position
	///   (e.g. char -&gt; int at a <c>return</c> boundary), when that differs from the literal's own
	///   natural type. Only applies between the numeric/char <see cref="SpecialType" />s that C#'s
	///   implicit numeric conversions actually cover - anything else (reference types, bool, string)
	///   is left alone.
	/// </summary>
	private bool TryCoerceLiteralToConvertedType(object literalValue, ExpressionSyntax originalExpression, out ExpressionSyntax? coerced)
	{
		coerced = null;

		var typeInfo = semanticModel.GetTypeInfo(originalExpression);

		if (typeInfo is not { Type: { } naturalType, ConvertedType: { } convertedType }
		    || SymbolEqualityComparer.Default.Equals(naturalType, convertedType)
		    || !IsCoercibleNumericSpecialType(naturalType.SpecialType)
		    || !IsCoercibleNumericSpecialType(convertedType.SpecialType))
		{
			return false;
		}

		return TryCreateLiteral(literalValue.ToSpecialType(convertedType.SpecialType), out coerced);
	}

	private static bool IsCoercibleNumericSpecialType(SpecialType specialType)
	{
		return specialType is SpecialType.System_Byte or SpecialType.System_SByte or SpecialType.System_Int16
			...
			or SpecialType.System_Double or SpecialType.System_Decimal or SpecialType.System_Char;
	}
