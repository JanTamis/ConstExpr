...
public class ConstExprPartialRewriter(SemanticModel semanticModel, MetadataLoader loader, Action<SyntaxNode?, Exception> exceptionHandler, IDictionary<string, VariableItem> variables, CancellationToken token) : CSharpSyntaxRewriter
{
	...
	public override SyntaxNode? VisitInvocationExpression(InvocationExpressionSyntax node)
	{
		    ...
		    && TryGetLiteralValue(instanceName, out var instance))
		{
			...
			var constantArguments = arguments
				.Where(w => TryGetConstantValue(semanticModel.Compilation, loader, w, new VariableItemDictionary(variables), token, out _))
				.Select(s => GetConstantValue(semanticModel.Compilation, loader, s, new VariableItemDictionary(variables), token))
				.ToArray();
	...
	{
		if (TryGetSymbol(node.Type, out INamedTypeSymbol? symbol))
		{
			var expression = Visit(node.Expression);
	...
	public override SyntaxNode? VisitElementAccessExpression(ElementAccessExpressionSyntax node)
	{
		...
		if (TryGetLiteralValue(instance, out var instanceValue))
		{
				...
				switch (operation)
				{
					...
					case IPropertyReferenceOperation propOp:
						if (propOp.Property.IsIndexer
						    ...
						    && TryGetLiteralValue(instance, out var instValue)
						    && constantArguments.Length == propOp.Arguments.Length)
						{
							try
							{
								if (loader.TryExecuteMethod(propOp.Property.GetMethod, instValue, new VariableItemDictionary(variables), constantArguments, out var value)
								    && TryGetLiteral(value, out var literal))
								{
									return literal;
								}
			...
			if (TryGetSymbol(node, out IPropertySymbol? propertySymbol)
	...
	private bool TryGetSymbol<TSymbol>(SyntaxNode? node, [NotNullWhen(true)] out TSymbol? value) where TSymbol : ISymbol
	{
		if (node is not null)
		{
			var info = semanticModel.GetSymbolInfo(node, token);

			if (info.Symbol is null)
			{
				if (semanticModel.Compilation.TryGetSemanticModel(node, out var semantic))
				{
					info = semantic.GetSymbolInfo(node, token);
				}
				else
				{
					value = default;
