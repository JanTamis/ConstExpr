...
///   The negation of GreaterThanOrEqualTrailingZeroCountStrategy's fold. Holds for signed
///   and unsigned types, including <c>x == 0</c> (TrailingZeroCount returns the bit width
///   there, so the mask test is also true). Safe under Strict.
/// </summary>
public class LessThanTrailingZeroCountStrategy : BaseBinaryStrategy
{
	...
	public override bool TryOptimize(BinaryOptimizeContext<ExpressionSyntax, ExpressionSyntax> context, out ExpressionSyntax? optimized)
	{
		...
		// count <= 0 is a contradiction and count >= the bit width means x != 0; both are left to other passes
