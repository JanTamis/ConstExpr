...
public class MaxByLinqUnroller : BaseLinqUnroller
{
	...
	public override void UnrollLoopBody(UnrolledLinqMethod method, List<StatementSyntax> statements, ref ExpressionSyntax elementName)
	{
		...
		statements.Add(CreateLocalDeclaration("key", keyExpr));
		...
		var condition = BinaryExpression(SyntaxKind.LogicalOrExpression,
			...
			BinaryExpression(SyntaxKind.GreaterThanExpression, IdentifierName("key"), IdentifierName(BestKeyName)));

		statements.Add(IfStatement(condition, Block(
			CreateAssignment(ResultName, elementName),
			CreateAssignment(BestKeyName, IdentifierName("key")),
