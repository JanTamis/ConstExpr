...
public class SinCosFunctionOptimizer : BaseFunctionOptimizer
{
	public override bool TryOptimize(IMethodSymbol method, FloatingPointEvaluationMode floatingPointMode, IList<ExpressionSyntax> parameters, IDictionary<SyntaxNode, bool> additionalMethods, out SyntaxNode? result)
	{
		...
		{
			// Generate fast sincos method for floating point types
			if (paramType.SpecialType is SpecialType.System_Single or SpecialType.System_Double)
			{
				...
				var fastSinCosMethod = ParseMethodFromString(methodString);
				
				if (fastSinCosMethod is not null)
				{
					if (!additionalMethods.ContainsKey(fastSinCosMethod))
					{
						additionalMethods.Add(fastSinCosMethod, false);
					}
					
					result = SyntaxFactory.InvocationExpression(
						SyntaxFactory.IdentifierName("FastSinCos"))
						.WithArgumentList(
		...
		result = CreateInvocation(paramType, "SinCos", parameters);
		return true;
	}
