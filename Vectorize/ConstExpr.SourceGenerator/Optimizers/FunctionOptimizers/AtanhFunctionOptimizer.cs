...
public class AtanhFunctionOptimizer : BaseFunctionOptimizer
{
	public override bool TryOptimize(IMethodSymbol method, FloatingPointEvaluationMode floatingPointMode, IList<ExpressionSyntax> parameters, IDictionary<SyntaxNode, bool> additionalMethods, out SyntaxNode? result)
	{
		...
		// Expect one parameter for Atanh
		...
		// When FastMath is enabled, add a fast atanh approximation method
		if (floatingPointMode == FloatingPointEvaluationMode.FastMath)
		{
			// Generate fast atanh method for floating point types
			if (paramType.SpecialType is SpecialType.System_Single or SpecialType.System_Double)
			{
				...
				var fastAtanhMethod = ParseMethodFromString(methodString);

				if (fastAtanhMethod is not null)
				{
					if (!additionalMethods.ContainsKey(fastAtanhMethod))
					{
						additionalMethods.Add(fastAtanhMethod, false);
					}

					result = SyntaxFactory.InvocationExpression(
						SyntaxFactory.IdentifierName("FastAtanh"))
						.WithArgumentList(
		...
		result = CreateInvocation(paramType, "Atanh", x);
		return true;
	}
