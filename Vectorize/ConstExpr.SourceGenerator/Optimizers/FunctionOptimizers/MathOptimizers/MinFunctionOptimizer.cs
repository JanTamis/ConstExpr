...
public class MinFunctionOptimizer() : BaseMathFunctionOptimizer("Min", n => n is 2)
{
	protected override bool TryOptimizeMath(FunctionOptimizerContext context, ITypeSymbol paramType, [NotNullWhen(true)] out SyntaxNode? result)
	{
		...
		// Min(x, T.MinValue) → T.MinValue (absorbing; dropping x's evaluation requires it to be pure)
		// Min(x, T.MaxValue) → x (identity; x is preserved, so no purity requirement)
	...
	private bool TryRewriteClampFromMinMax(ITypeSymbol paramType, string? outerContainingName, ExpressionSyntax first, ExpressionSyntax second, out InvocationExpressionSyntax? result)
	{
		result = null;
	...
	// constSide is checked for being a known T.MinValue/T.MaxValue literal; otherSide is the other Min(...) operand.
	// Min(x, T.MinValue) -> T.MinValue (absorbing: x's evaluation is dropped, so it must be pure)
	// Min(x, T.MaxValue) -> x (identity: x is preserved unconditionally, so no purity requirement)
	...
	// free, so a nested Min/Max call (e.g. the inner Byte.Min(r, g) of Byte.Min(Byte.Min(r, g), 0)) is
