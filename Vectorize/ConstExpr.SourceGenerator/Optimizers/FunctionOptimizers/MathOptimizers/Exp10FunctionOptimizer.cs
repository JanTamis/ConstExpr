...
public class Exp10FunctionOptimizer() : BaseMathFunctionOptimizer("Exp10", n => n is 1)
{
	...
	private static string GenerateFastExp10MethodFloat(FastMathFlags flags)
	{
		...
		builder.WriteLine("if (Single.IsPositiveInfinity(x)) return Single.PositiveInfinity;")
			...
			.WriteLine("if (x == 0.0f) return 1.0f; // handles +0 and -0")
			.WriteLine("")
			...
			.WriteLine("")
			.WriteLine("var kf = x * LOG2_10;")
			.WriteLine("var k  = (int)(kf + (kf >= 0.0f ? 0.5f : -0.5f));")
			.WriteLine("var r  = Single.FusedMultiplyAdd(-k, LOG10_2, x);")
			.WriteLine("")
			...
			.WriteLine("const float c3 = 2.0346786f;  // ln(10)³ / 6")
			.WriteLine("const float c2 = 2.6509491f;  // ln(10)² / 2")
			.WriteLine("const float c1 = 2.3025851f;  // ln(10)")
			.WriteLine("")
			.WriteLine("var poly = Single.FusedMultiplyAdd(c4, r, c3);")
			.WriteLine("poly = Single.FusedMultiplyAdd(poly, r, c2);")
			.WriteLine("poly = Single.FusedMultiplyAdd(poly, r, c1);")
	...
	private static string GenerateFastExp10MethodDouble(FastMathFlags flags)
	{
		...
		builder.WriteLine("if (Double.IsPositiveInfinity(x)) return Double.PositiveInfinity;")
			...
			.WriteLine("if (x == 0.0) return 1.0; // handles +0 and -0")
			.WriteLine("")
			...
			.WriteLine("var r  = System.Math.FusedMultiplyAdd(-k, LOG10_2, x);")
			.WriteLine("")
			.WriteLine("// Degree-4 Horner for 10^r: cₙ = ln(10)ⁿ / n!")
			.WriteLine("// Max relative error ≈ 4e-5 (fast-math trade-off).")
			...
			.WriteLine("")
			.WriteLine("var poly = Double.FusedMultiplyAdd(c4, r, c3);")
			.WriteLine("poly = Double.FusedMultiplyAdd(poly, r, c2);")
			.WriteLine("poly = Double.FusedMultiplyAdd(poly, r, c1);")
