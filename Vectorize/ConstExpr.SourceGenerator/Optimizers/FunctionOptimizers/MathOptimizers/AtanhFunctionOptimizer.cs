...
public class AtanhFunctionOptimizer() : BaseMathFunctionOptimizer("Atanh", n => n is 1), IBaseMathCustomImplementation
{
	...
	public override string GenerateCustomImplementation(FunctionOptimizerContext context, ITypeSymbol paramType)
	{
		...
		{
			SpecialType.System_Single => GenerateFastAtanhMethodFloat(context, paramType),
			SpecialType.System_Double => GenerateFastAtanhMethodDouble(context, paramType),
			_ => null
		});
	...
	private static string GenerateFastAtanhMethodFloat(FunctionOptimizerContext context, ITypeSymbol paramType)
	{
		...
		builder.WriteLine("var absX = Single.Abs(x);")
			...
			.WriteLine($"var p = {multiplyAdd("x2", 1f / 7f, 1f / 5f)};")
			.WriteLine($"p = {multiplyAdd("p", "x2", 1f / 3f)};")
			.WriteLine($"p = {multiplyAdd("p", "x2", 1f)};")
			...
			.WriteLine($"return 0.5f * {multiplyAdd("e", 0.6931471806f, "lnm")};");
	...
	private static string GenerateFastAtanhMethodDouble(FunctionOptimizerContext context, ITypeSymbol paramType)
	{
		var builder = new CodeWriter();
		var multiplyAdd = MultiplyAddEstimate(context, paramType);
		...
		builder.WriteLine("if (Math.Abs(x) >= 1.0) return x > 0 ? Double.PositiveInfinity : Double.NegativeInfinity;")
			...
			.WriteLine("var x2 = x * x;")
			.WriteWhitespace()
			.WriteLine($"var poly = {multiplyAdd("x2", 1d / 11d, 1d / 9d)};")
			.WriteLine($"poly = {multiplyAdd("poly", "x2", 1d / 7d)};")
			.WriteLine($"poly = {multiplyAdd("poly", "x2", 1d / 5d)};")
			.WriteLine($"poly = {multiplyAdd("poly", "x2", 1d / 3d)};")
			.WriteLine($"poly = {multiplyAdd("poly", "x2", 1d)};")
			.WriteWhitespace()
			...
			.WriteLine($"return 0.5 * {multiplyAdd("e", 0.6931471805599453094172321214581766, "lnm")};")
			.EndBlock();
