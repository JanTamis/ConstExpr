...
public class AsinPiFunctionOptimizer : BaseFunctionOptimizer
{
	public override bool TryOptimize(IMethodSymbol method, FloatingPointEvaluationMode floatingPointMode, IList<ExpressionSyntax> parameters, IDictionary<SyntaxNode, bool> additionalMethods, out SyntaxNode? result)
	{
		...
		// When FastMath is enabled, add a fast asinpi approximation method
		if (floatingPointMode == FloatingPointEvaluationMode.FastMath)
		{
			// Generate fast asinpi method for floating point types
			if (paramType.SpecialType is SpecialType.System_Single or SpecialType.System_Double)
			{
				...
				var fastAsinPiMethod = ParseMethodFromString(methodString);
				
				if (fastAsinPiMethod is not null)
				{
					if (!additionalMethods.ContainsKey(fastAsinPiMethod))
					{
						additionalMethods.Add(fastAsinPiMethod, false);
					}
					
					result = SyntaxFactory.InvocationExpression(
						SyntaxFactory.IdentifierName("FastAsinPi"))
						.WithArgumentList(
		...
		result = CreateInvocation(paramType, "AsinPi", parameters);
		return true;
	}
