...
public class IsPow2FunctionOptimizer() : BaseBitOperationsFunctionOptimizer("IsPow2", n => n is 1)
{
	protected override bool TryOptimizeMath(FunctionOptimizerContext context, ITypeSymbol paramType, [NotNullWhen(true)] out SyntaxNode? result)
	{
		...
		//
		// 	// x != 0u && (x & (x - 1u)) == 0u
		// 	var notZero = NotEqualsExpression(arg, zero);
		// 	var xMinusOne = SubtractExpression(arg, one);
		// 	var andExpr = BitwiseAndExpression(arg, ParenthesizedExpression(xMinusOne));
		// 	var eqZero = EqualsExpression(ParenthesizedExpression(andExpr), zero);
		//
		// 	result = LogicalAndExpression(notZero, eqZero);
		// 	return true;
		// }
		//
		...
		//
		// 	// x > 0 && (x & (x - 1)) == 0
		// 	var gtZero = GreaterThanExpression(arg, zero);
		// 	var xMinusOne = SubtractExpression(arg, one);
		// 	var andExpr = BitwiseAndExpression(arg, ParenthesizedExpression(xMinusOne));
		// 	var eqZero = EqualsExpression(ParenthesizedExpression(andExpr), zero);
		//
		// 	result = LogicalAndExpression(gtZero, eqZero);
		// 	return true;
		// }
	...
	private static ExpressionSyntax MakeUnsignedLiteral(ITypeSymbol type, int value)
	{
		return type.SpecialType == SpecialType.System_UInt64
			? CreateLiteral((ulong)value)
			: CreateLiteral((uint)value);
	}
}