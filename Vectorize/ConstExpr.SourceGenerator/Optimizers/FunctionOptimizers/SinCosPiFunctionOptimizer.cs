...
public class SinCosPiFunctionOptimizer : BaseFunctionOptimizer
{
	public override bool TryOptimize(IMethodSymbol method, FloatingPointEvaluationMode floatingPointMode, IList<ExpressionSyntax> parameters, IDictionary<SyntaxNode, bool> additionalMethods, out SyntaxNode? result)
	{
		...
		{
			// Generate fast sincospi method for floating point types
			if (paramType.SpecialType is SpecialType.System_Single or SpecialType.System_Double)
			{
				...
				var fastSinCosPiMethod = ParseMethodFromString(methodString);
				
				if (fastSinCosPiMethod is not null)
				{
					if (!additionalMethods.ContainsKey(fastSinCosPiMethod))
					{
						additionalMethods.Add(fastSinCosPiMethod, false);
					}
					
					result = SyntaxFactory.InvocationExpression(
						SyntaxFactory.IdentifierName("FastSinCosPi"))
						.WithArgumentList(
		...
		result = CreateInvocation(paramType, "SinCosPi", parameters);
		return true;
	}
