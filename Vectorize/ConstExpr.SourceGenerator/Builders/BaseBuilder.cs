...
public abstract class BaseBuilder(ITypeSymbol elementType, Compilation compilation, GenerationLevel generationLevel, MetadataLoader loader, int hashCode)
{
	...
	protected void AppendMethod<T>(IndentedStringBuilder builder, IMethodSymbol methodSymbol, ReadOnlySpan<T> items, VectorTypes type, bool isRepeating, Action<IList<string>, int> vectorAction, Action<bool> action)
	{
		using (AppendMethod(builder, methodSymbol))
		{
			var isPerformance = IsPerformance(generationLevel, items.Length);
	...
	protected void GenerateCodeFromTree<T>(IndentedStringBuilder builder, TreeNode<T> node, string compareFormat, bool isFirst, IMethodSymbol method, IList<KeyValuePair<T, int>> items)
	{
		if (node.IsLeaf)
		{
