...
{
	/// <summary>Analyzes a <see cref="RegexTree"/> to learn about the structure of the tree.</summary>
	public static AnalysisResults Analyze(RegexTree regexTree)
	{
		var results = new AnalysisResults(regexTree);
		results._complete = TryAnalyze(regexTree.Root, results, true, false);
		return results;
...
public sealed class AnalysisResults
{
	...
	{
		return !_complete || _containsCapture.Contains(node);
	}
	...
	{
		return !_complete || (_mayBacktrack?.Contains(node) ?? false);
	}
	...
	{
		return !_complete || (_inLoops?.Contains(node) ?? false);
	}
