...
#pragma warning restore RS1035
	...
	public static RegexTree Parse(string pattern, RegexOptions options, CultureInfo culture)
	{
		...
		else
		{
			...
			for (var i = 0; i < captureNumberList.Length; i++)
			{
...
#pragma warning restore RS1035
	...
	{
		var indexOfMetachar = IndexOfAnyMetachar(input.AsSpan());
	...
	private static string EscapeImpl(ReadOnlySpan<char> input, int indexOfMetachar)
	{
		...
		while (true)
		{
			...
			indexOfMetachar = IndexOfAnyMetachar(input);
	...
	private RegexNode ScanRegex()
	{
		...
		while (_pos < _pattern.Length)
		{
			...
			switch (ch)
			{
				...
				case '[':
				{
					var setString = ScanCharClass((_options & RegexOptions.IgnoreCase) != 0, false)!.ToStringClass();
				...
				switch (ch)
				{
					...
					case '{':
						...
						if (startpos < _pos)
						{
							...
							{
								_pos++;
								max = _pos == _pattern.Length || _pattern[_pos] == '}' ? Int32.MaxValue : ScanDecimal();
							}
						}
	...
	private RegexNode ScanReplacement()
	{
		...
		while (_pos < _pattern.Length)
		{
			...
			AddToConcatenate(startpos, _pos - startpos, true);
	...
	private char ScanOctal()
	{
		...
		var c = Math.Min(3, _pattern.Length - _pos);
		int d;
		int i;
	...
	private char ScanHex(int c)
	{
		...
		if (_pos + c <= _pattern.Length)
		{
			...
			{
				var ch = _pattern[_pos++];
				var result = HexConverter.FromChar(ch);
	...
	{
		for (var off = false; _pos < _pattern.Length; _pos++)
		{
			var ch = _pattern[_pos];
			...
			{
				var options = (char) (ch | 0x20) switch
				{
	...
	private string ParseProperty()
	{
		...
		var capname = _pattern.Substring(startpos, _pos - startpos);
	...
	{
		var lookaheadOpts = _options & ~RegexOptions.RightToLeft;
		var lookbehindOpts = _options | RegexOptions.RightToLeft;
		var lookaheadOptsNoCase = lookaheadOpts & ~RegexOptions.IgnoreCase;
		var lookbehindOptsNoCase = lookbehindOpts & ~RegexOptions.IgnoreCase;
	...
	{
		var opts = _options;
		var lookaheadOpts = opts & ~RegexOptions.RightToLeft;
		var lookaheadOptsNoCase = lookaheadOpts & ~RegexOptions.IgnoreCase;
	...
	{
		var opts = _options;
		var lookbehindOpts = opts | RegexOptions.RightToLeft;
		var lookbehindOptsNoCase = lookbehindOpts & ~RegexOptions.IgnoreCase;
	...
	private void NoteCaptureSlot(int i, int pos)
	{
		...
		if (!_caps.ContainsKey(boxedI))
		{
			...
			if (_captop <= i)
			{
				_captop = i == Int32.MaxValue ? i : i + 1;
			}
		}
	}
	...
	private static int IndexOfAnyMetachar(ReadOnlySpan<char> span)
	{
		// Metacharacters used in regex that need to be escaped: \t \n \f \r space # $ ( ) * + . ? [ \ ^ { |
	...
	private readonly bool IsTrueQuantifier()
	{
		...
		var pos = startpos;
		var nChars = _pattern.Length - _pos;
	...
	{
		return new RegexParseException(error, _pos, SR.Format(SR.MakeException, _pattern, _pos, message));
	}
