...
internal sealed partial class RegexCharClass
{
	...
	public void AddCharClass(RegexCharClass cc)
	{
		...
		var ccRangeCount = cc._rangelist?.Count ?? 0;
	...
	{
		return _categories ??= new StringBuilder();
	}

	private List<(char First, char Last)> EnsureRangeList()
	{
		return _rangelist ??= new List<(char First, char Last)>(6);
	}
	...
	public void AddCaseEquivalences(CultureInfo culture)
	{
		...
		if (rangeList != null)
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				{
					if (RegexCaseEquivalences.TryFindCaseEquivalencesForCharWithIBehavior(range.First, culture, ref _caseBehavior, out var equivalences))
					{
	...
	{
		var startsWithNulls = set.Length >= 2 && set[0] == '\0' && set[1] == '\0';
		var strLength = SetStartIndex + set.Length + category.Length;
	...
	public static bool TryGetOnlyCategories(string set, Span<UnicodeCategory> categories, out int numCategories, out bool negated)
	{
		...
		var categoryEnd = SetStartIndex + set[CategoryLengthIndex];

		for (var pos = SetStartIndex; pos < categoryEnd; pos++)
		{
	...
	public static int GetSetChars(string set, Span<char> chars)
	{
		...
		for (var i = SetStartIndex; i < SetStartIndex + setLength; i += 2)
		{
	...
	public static bool MayOverlap(string set1, string set2)
	{
		...
		var set1Negated = IsNegated(set1);
		var set2Negated = IsNegated(set2);
		...
		if (TryGetOnlyCategories(set1, categories1, out var numCategories1, out var negated1) &&
		    TryGetOnlyCategories(set2, categories2, out var numCategories2, out var negated2))
		{
			...
			foreach (var cat1 in categories1.Slice(0, numCategories1))
			{
				foreach (var cat2 in categories2.Slice(0, numCategories2))
				{
					if (cat1 == cat2)
					{
						return true;
		...
		static bool MayOverlapByEnumeration(string set1, string set2)
		{
			...
			for (var i = SetStartIndex; i < SetStartIndex + set2[SetLengthIndex]; i += 2)
			{
	...
	public static bool IsSubsetOf(string subset, string superset)
	{
		...
		{
			for (var i = SetStartIndex; i < SetStartIndex + subset[SetLengthIndex]; i += 2)
			{
		...
		if (TryGetOnlyCategories(subset, categories1, out var numCategories1, out var negated1) && !negated1 &&
		    TryGetOnlyCategories(superset, categories2, out var numCategories2, out var negated2) && !negated2)
		{
			foreach (var cat1 in categories1.Slice(0, numCategories1))
			{
				var found = false;
	...
	{
		return CanEasilyEnumerateSetContents(set, out var hasSubtraction) &&
		       !hasSubtraction;
	}
	...
	internal static CharClassAnalysisResults Analyze(string set)
	{
		...
		var firstValueInclusive = set[SetStartIndex];
		var lastValueExclusive = set[SetStartIndex + set[SetLengthIndex] - 1];
	...
	{
		return charClass.Length > SetStartIndex +
	...
	{
		return set[FlagsIndex] == 1;
	}

	internal static bool IsNegated(string set, int setOffset)
	{
		return set[FlagsIndex + setOffset] == 1;
	}
	...
	public static bool IsECMAWordChar(char ch)
	{
		...
		return ((uint) ch - 'A' & ~0x20) < 26 || // ASCII letter
		       (uint) ch - '0' < 10 || // digit
		       ch == '_' || // underscore
		       ch == '\u0130';
	...
	{
		return (WordCategoriesMask & 1 << (int) category) != 0;
	}
	...
	{
		return CharInClassIterative(ch, set, 0);
	}
	...
	public static RegexCharClass Parse(string charClass)
	{
		...
		while (true)
		{
			...
			var endPosition = pos + SetStartIndex + setLength + categoryLength;

			var ranges = ComputeRanges(charClass.AsSpan(pos));

			StringBuilder? categoriesBuilder = null;

			if (categoryLength > 0)
			{
				var end = pos + SetStartIndex + setLength;
				categoriesBuilder = new StringBuilder().Append(charClass, end, categoryLength);
			}

			var level = new RegexCharClass(IsNegated(charClass, pos), ranges, categoriesBuilder, null);
	...
	public static List<(char First, char Last)>? ComputeRanges(ReadOnlySpan<char> set)
	{
		...
		if (setLength > 0)
		{
			...
			while (i < end)
			{
				...
				var last = i < end ? (char) (set[i] - 1) : LastChar;
				i++;
	...
	internal static string CharsToStringClass(ReadOnlySpan<char> chars)
	{
		...
		switch (chars.Length)
		{
			...
			case 1:
				...
				if (chars[0] < 128)
				{
					var asciiStrings = s_asciiStrings;
		...
		var tmpChars = chars; // avoid address exposing the span and impacting the other code in the method that uses it
#if NET
		...
		unsafe
		{
			return StringExtensions.Create(SetStartIndex + count, (IntPtr) (&tmpChars), static (span, charsPtr) =>
			{
				...
				span[CategoryLengthIndex] = (char) 0;
				var i = SetStartIndex;
				var chars = *(ReadOnlySpan<char>*) charsPtr;
	...
	public string ToStringClass()
	{
		...
		do
		{
			...
			var categoriesLength = current._categories?.Length ?? 0;
			var headerSpan = vsb.AppendSpan(SetStartIndex);
			...
			// Append ranges
			var rangelist = current._rangelist;

			if (rangelist != null)
			{
				for (var i = 0; i < rangelist.Count; i++)
				{
					var currentRange = rangelist[i];
			...
			{
				vsb.Append(current._categories!.ToString().AsSpan());
			}
	...
	private void Canonicalize()
	{
		...
		if (rangelist != null)
		{
			...
			{
				var vsb = new ValueStringBuilder(stackalloc char[256]);
				vsb.Append(_categories!.ToString().AsSpan());
				...
				else
				{
					_negate = true;
					rangelist.RemoveAt(1);
					var notOne = (char) (rangelist[0].Last + 1);
	...
	private static ReadOnlySpan<char> RangesFromProperty(string capname, bool invert, string pattern, int currentPos)
	{
		...
		{
			var mid = (min + max) / 2;
			var res = String.Compare(capname, s_propTable[mid][0], StringComparison.Ordinal);
	...
	public static string DescribeSet(string set, bool forceBrackets = false)
	{
		int setLength = set[SetLengthIndex];
		int categoryLength = set[CategoryLengthIndex];
		var endPosition = SetStartIndex + setLength + categoryLength;
		var negated = IsNegated(set);
		...
		var index = SetStartIndex;
		char ch1;
		char ch2;
		var desc = new StringBuilder().Append('[');

		void RenderRanges()
		{
			var rangesEnd = SetStartIndex + set[SetLengthIndex];
		...
		while (index < SetStartIndex + set[SetLengthIndex] + set[CategoryLengthIndex])
		{
			...
			if (ch1 == 0)
			{
				...
				var lastindex = set.IndexOf(GroupChar, index + 1);

				if (lastindex != -1)
				{
					var group = set.AsSpan(index, lastindex - index + 1);
					...
					switch (group)
					{
						...
						default:
							...
							var invertedGroup = group.Length <= scratch.Length ? scratch.Slice(0, group.Length) : new char[group.Length];
							...
							{
								var equalsGroup = group.SequenceEqual(kvp.Value.AsSpan());
		...
		if (set.Length > endPosition)
		{
			desc.Append('-').Append(DescribeSet(set.Substring(endPosition), true));
		}
