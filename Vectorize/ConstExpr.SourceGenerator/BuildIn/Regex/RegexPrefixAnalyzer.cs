...
namespace Vectorize.ConstExpr.SourceGenerator.BuildIn
{
	...
	public static class RegexPrefixAnalyzer
	{
		...
		public static string[]? FindPrefixes(RegexNode node, bool ignoreCase)
		{
			...
			// Return the prefixes.
			var resultStrings = new string[results.Count];
			...
			static bool FindPrefixesCore(RegexNode node, List<StringBuilder> results, bool ignoreCase)
			{
				...
				while (true)
				{
					switch (node.Kind)
					{
						...
						{
							var charCount = RegexCharClass.GetSetChars(node.Str!, setChars);
							...
							if (!ignoreCase)
							{
								for (var rep = 0; rep < reps; rep++)
								{
									...
									foreach (var suffix in setChars.Slice(1, charCount - 1))
									{
						...
						{
							var limit = Math.Min(node.M, MaxPrefixLength); // MaxPrefixLength here is somewhat arbitrary, as a single loop iteration could yield multiple chars
						...
						case RegexNodeKind.Alternate:
						{
							...
							for (var i = 0; i < childCount; i++)
							{
								...
								foreach (var sb in alternateBranchResults)
								{
							...
							else
							{
								...
								for (var i = 1; i < allBranchResults!.Count; i++)
								{
									var suffix = allBranchResults[i];
			...
			static bool Process(RegexNode node, ref ValueStringBuilder vsb)
			{
				...
				switch (node.Kind)
				{
					...
					case RegexNodeKind.Alternate when !rtl: // for RTL we'd need to be matching the suffixes of the alternation cases
					{
						...
						var addedLength = vsb.Length - initialLength;
						...
						if (addedLength != 0)
						{
							...
							for (var i = 1; i < childCount && addedLength != 0; i++)
							{
					...
					case RegexNodeKind.Oneloop or RegexNodeKind.Oneloopatomic or RegexNodeKind.Onelazy when node.M > 0:
						...
						var count = Math.Min(node.M, SingleCharIterationLimit);
					...
					case RegexNodeKind.Loop or RegexNodeKind.Lazyloop when node.M > 0:
					{
						...
						var limit = Math.Min(node.M, NodeIterationLimit);
		...
		public static string? FindPrefixOrdinalCaseInsensitive(RegexNode node)
		{
			while (true)
			{
				...
				switch (node.Kind)
				{
					...
					case RegexNodeKind.Concatenate:
						node.TryGetOrdinalCaseInsensitiveString(0, node.ChildCount(), out _, out var caseInsensitiveString, consumeZeroWidthNodes: true, forPrefixAnalysis: true);
		...
		public static List<RegexFindOptimizations.FixedDistanceSet>? FindFixedDistanceSets(RegexNode root, bool thorough)
		{
			...
			for (var i = 0; i < results.Count; i++)
			{
				...
				if (RegexCharClass.TryGetSingleRange(result.Set, out var lowInclusive, out var highInclusive) &&
				...
				{
					var count = RegexCharClass.GetSetChars(result.Set, scratch);
			...
			static bool TryFindRawFixedSets(RegexNode node, List<RegexFindOptimizations.FixedDistanceSet> results, ref int distance, bool thorough)
			{
				...
				switch (node.Kind)
				{
					case RegexNodeKind.One:
						...
						{
							var setString = RegexCharClass.OneToStringClass(node.Ch);
					...
					{
						var setString = RegexCharClass.OneToStringClass(node.Ch);
						var minIterations = Math.Min(node.M, MaxLoopExpansion);
						var i = 0;
					...
					case RegexNodeKind.Multi:
					{
						...
						{
							var setString = RegexCharClass.OneToStringClass(s[i]);
							results.Add(new RegexFindOptimizations.FixedDistanceSet(null, setString, distance++));
						}
						return i == s.Length;
					}
					...
					case RegexNodeKind.Alternate when thorough:
					{
						...
						for (var i = 0; i < childCount; i++)
						{
							...
							foreach (var fixedSet in localResults)
							{
								if (combined.TryGetValue(fixedSet.Distance, out var value))
								{
		...
		public static void SortFixedDistanceSetsByQuality(List<RegexFindOptimizations.FixedDistanceSet> results) =>
			...
			{
				var s1Chars = s1.Chars;
				var s2Chars = s2.Chars;
				var s1CharsLength = s1Chars?.Length ?? 0;
				var s2CharsLength = s2Chars?.Length ?? 0;
				var s1Negated = s1.Negated;
				var s2Negated = s2.Negated;
				var s1RangeLength = s1.Range is not null ? GetRangeLength(s1.Range.Value, s1Negated) : 0;
				var s2RangeLength = s2.Range is not null ? GetRangeLength(s2.Range.Value, s2Negated) : 0;
				...
				if (!s1Negated)
				{
					...
					if (s1Chars is not null && s2Chars is not null)
					{
						...
						var s1Frequency = SumFrequencies(s1Chars);
						var s2Frequency = SumFrequencies(s2Chars);
					...
					{
						var c = Math.Max(s1CharsLength, s1RangeLength).CompareTo(Math.Max(s2CharsLength, s2RangeLength));
				...
				{
					var length = range.HighInclusive - range.LowInclusive + 1;
					return negated ?
			...
			static bool? TryFindFirstOrLastCharClass(RegexNode node, bool findFirst, ref RegexCharClass? cc)
			{
				...
				switch (node.Kind)
				{
					...
					case RegexNodeKind.Multi:
						...
						{
							cc ??= new RegexCharClass();
							var firstChar = findFirst == ((node.Options & RegexOptions.RightToLeft) == 0);
					...
					case RegexNodeKind.Concatenate:
					{
						...
						if (findFirst)
						{
							for (var i = 0; i < childCount; i++)
							{
								var childResult = TryFindFirstOrLastCharClass(node.Child(i), findFirst, ref cc);
					...
					case RegexNodeKind.ExpressionConditional:
						var branchStart = node.Kind is RegexNodeKind.BackreferenceConditional ? 0 : 1;
			...
			while (true)
			{
				switch (node.Kind)
				{
						...
						// looking for the last we shouldn't skip.
					{
						...
						var bestAnchorFound = RegexNodeKind.Unknown;

						if (leading)
						{
							for (var i = 0; i < childCount; i++)
							{
								var tmpChild = node.Child(i);
						...
						// For alternations, every branch needs to lead or trail with the same anchor.
					{
						...
						var anchor = FindLeadingOrTrailingAnchor(node.Child(0), leading);
