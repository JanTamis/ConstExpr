...
/// <summary>Provides optimizations for finding the starting position of a regex match.</summary>
...
public sealed class RegexFindOptimizations
{
	...
	{
		var rightToLeft = (options & RegexOptions.RightToLeft) != 0;
		...
		var prefix = RegexPrefixAnalyzer.FindPrefix(root);
		...
		var dfa = (options & RegexOptions.NonBacktracking) != 0;
		var compiled = (options & RegexOptions.Compiled) != 0 && !dfa; // for now, we never generate code for NonBacktracking, so treat it as non-compiled
		var interpreter = !compiled && !dfa;
		...
		var fixedDistanceSets = RegexPrefixAnalyzer.FindFixedDistanceSets(root, thorough: !interpreter);
		...
		var literalAfterLoop = RegexPrefixAnalyzer.FindLiteralFollowingLeadingLoop(root);
	...
	private static (string String, int Distance)? FindFixedDistanceString(List<FixedDistanceSet> fixedDistanceSets)
	{
		...
		if (fixedDistanceSets.Count >= 2)
		{
			...
			for (var i = 0; i < fixedDistanceSets.Count + 1; i++)
			{
				var chars = i < fixedDistanceSets.Count ? fixedDistanceSets[i].Chars : null;
				var invalidChars = chars is not { Length: 1 } || fixedDistanceSets[i].Negated;
	...
	private static bool HasHighFrequencyChars(FixedDistanceSet set)
	{
		...
		var frequency = RegexPrefixAnalyzer.Frequency;
