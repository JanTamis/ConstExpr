...
public sealed class RegexNode
{
	...
	public RegexNode CloneCharLoopWithOneLessIteration()
	{
		...
		var newNode = IsSetFamily ?
			new RegexNode(Kind, Options, Str!) :
			new RegexNode(Kind, Options, Ch);

		newNode.M = M - 1;
		newNode.N = N == Int32.MaxValue ? Int32.MaxValue : N - 1;
	...
	public static RegexNode CreateOneWithCaseConversion(char ch, RegexOptions options, CultureInfo? culture, ref RegexCaseBehavior caseBehavior)
	{
		...
		if ((options & RegexOptions.IgnoreCase) != 0)
		{
			...
			var stringSet = RegexCharClass.CharsToStringClass(equivalences);
	...
	internal RegexNode FinalOptimize()
	{
		...
		if ((rootNode.Options & (RegexOptions.RightToLeft | RegexOptions.NonBacktracking)) == 0)
		{
			...
			{
				var node = rootNode.Child(0); // skip implicit root capture node
				var atomicByAncestry = true; // the root is implicitly atomic because nothing comes after it (same for the implicit root capture)
				...
				while (true)
				{
					switch (node.Kind)
					{
						...
						case RegexNodeKind.Oneloop or RegexNodeKind.Oneloopatomic or RegexNodeKind.Notoneloop or RegexNodeKind.Notoneloopatomic or RegexNodeKind.Setloop or RegexNodeKind.Setloopatomic when node.N == Int32.MaxValue:
						case RegexNodeKind.Onelazy or RegexNodeKind.Notonelazy or RegexNodeKind.Setlazy when node.N == Int32.MaxValue && !atomicByAncestry:
	...
	private void EliminateEndingBacktracking()
	{
		...
		while (true)
		{
			...
			var rtl = (node.Options & RegexOptions.RightToLeft) != 0;
			...
			switch (node.Kind)
			{
				...
				case RegexNodeKind.Concatenate when !rtl:
					var existingChild = node.Child(node.ChildCount() - 1);
				...
				case RegexNodeKind.Loop:
				{
					...
					{
						if (node.N == 1 || CanBeMadeAtomic(node.Child(0), node.Child(0), false, false))
						{
	...
	private RegexNode ReduceAtomic()
	{
		...
		switch (child.Kind)
		{
			...
			case RegexNodeKind.Alternate:
				if ((Options & RegexOptions.RightToLeft) == 0)
				{
					var branches = child.Children as List<RegexNode>;
					...
					for (var i = 1; i < branches.Count - 1; i++)
					{
					...
					for (var start = 0; start < branches.Count; start++)
					{
						...
						if (endExclusive - start >= 3)
						{
							...
							while (compare < endExclusive)
							{
								...
								var c = branches[compare].FindBranchOneOrMultiStart()!.FirstCharOfOneOrMulti();
								...
								if (compare < endExclusive)
								{
									...
									for (var next = compare + 1; next < endExclusive; next++)
									{
										var nextChild = branches[next];
	...
	private RegexNode ReduceLoops()
	{
		...
		while (u.ChildCount() > 0)
		{
			...
			if (u.M > 0)
			{
				u.M = min = (Int32.MaxValue - 1) / u.M < min ? Int32.MaxValue : u.M * min;
			}

			if (u.N > 0)
			{
				u.N = max = (Int32.MaxValue - 1) / u.N < max ? Int32.MaxValue : u.N * max;
			}
		}
	...
	private RegexNode ReduceAlternation()
	{
		...
		switch (ChildCount())
		{
			...
			default:
				...
				var node = ReplaceNodeIfUnnecessary();
					...
					if (node.Kind == RegexNodeKind.Alternate)
					{
						...
						if (node.Kind == RegexNodeKind.Alternate)
						{
							...
							if (node.Kind is RegexNodeKind.Alternate && node.ChildCount() == 2)
							{
								...
								{
									node = node.Child(0).MakeQuantifier(false, 0, 1);
								}
								else if (node.Child(0).Kind is RegexNodeKind.Empty)
								{
									node = node.Child(1).MakeQuantifier(true, 0, 1);
								}
							}
						}
					}
		...
		{
			var wasLastSet = false;
			var lastNodeCannotMerge = false;
			...
			for (i = 0, j = 0; i < children.Count; i++, j++)
			{
				...
				while (true)
				{
					if (at.Kind == RegexNodeKind.Alternate)
					{
						...
						{
							for (var k = 0; k < atChildren.Count; k++)
							{
						...
						{
							var atChild = (RegexNode) at.Children!;
							atChild.Parent = this;
					...
					else if (at.Kind is RegexNodeKind.Set or RegexNodeKind.One)
					{
						...
						{
							var atCharClass = RegexCharClass.Parse(at.Str!);
		...
		static RegexNode ExtractCommonPrefixNode(RegexNode alternation)
		{
			...
			for (var startingIndex = 0; startingIndex < children.Count - 1; startingIndex++)
			{
				...
				var startingNode = children[startingIndex];
				var required = startingNode.Kind == RegexNodeKind.Concatenate ? startingNode.Child(0) : startingNode;
				...
				{
					var endingNode = children[endingIndex];
					var other = endingNode.Kind == RegexNodeKind.Concatenate ? endingNode.Child(0) : endingNode;
		...
		static RegexNode ExtractCommonPrefixText(RegexNode alternation)
		{
			...
			for (var startingIndex = 0; startingIndex < children.Count - 1; startingIndex++)
			{
				...
				var startingNode = children[startingIndex].FindBranchOneOrMultiStart();
				...
				var startingNodeOptions = startingNode.Options;
				scoped var startingSpan = startingNode.Str.AsSpan();
				...
				for (; endingIndex < children.Count; endingIndex++)
				{
					...
					else
					{
						...
						var minLength = Math.Min(startingSpan.Length, startingNode.Str.Length);
						var c = 0;
	...
	{
		var branch = Kind == RegexNodeKind.Concatenate ? Child(0) : this;
	...
	public bool TryGetAlternationStartingChars([NotNullWhen(true)] out HashSet<char>? seenChars)
	{
		...
		for (var i = 0; i < childCount; i++)
		{
			...
			else
			{
				...
				foreach (var c in setChars.Slice(0, numChars))
				{
					if (!seenChars.Add(c))
					{
						seenChars = null;
			...
			switch (node.Kind)
			{
				...
				case RegexNodeKind.Set or RegexNodeKind.Setloop or RegexNodeKind.Setloopatomic or RegexNodeKind.Setlazy:
					    ...
					    highInclusive - lowInclusive > 1) // prefer IndexOfAny for 1 or 2 elements as an optimization
					{
						...
						return new StartingLiteralData((lowInclusive, highInclusive), RegexCharClass.IsNegated(node.Str!));
					}
					...
					{
						return new StartingLiteralData(setChars.Slice(0, numChars).ToString(), RegexCharClass.IsNegated(node.Str!));
					}
					break;
	...
	private void ReduceConcatenationWithAdjacentStrings()
	{
		...
		for (i = 0, j = 0; i < children.Count; i++, j++)
		{
			...
			else if (at.Kind is RegexNodeKind.Multi or RegexNodeKind.One)
			{
				...
				var optionsAt = at.Options & (RegexOptions.RightToLeft | RegexOptions.IgnoreCase);
				...
				prev.Str = (optionsAt & RegexOptions.RightToLeft) == 0 ? at.Kind == RegexNodeKind.One ? $"{prev.Str}{at.Ch}" : prev.Str + at.Str :
		...
		while (next < children.Count)
		{
			...
			if (currentNode.Options == nextNode.Options)
			{
				static bool CanCombineCounts(int nodeMin, int nodeMax, int nextMin, int nextMax)
				{
					...
					if (nodeMin == Int32.MaxValue ||
					    nextMin == Int32.MaxValue ||
					    (uint) nodeMin + (uint) nextMin >= Int32.MaxValue)
					{
						return false;
					}

					// Similar overflow / go infinite check for max (which can be infinite).
					if (nodeMax != Int32.MaxValue &&
					    nextMax != Int32.MaxValue &&
					    (uint) nodeMax + (uint) nextMax >= Int32.MaxValue)
					{
						return false;
					}
				...
				switch (currentNode.Kind)
				{
					...
					case RegexNodeKind.Setloop or RegexNodeKind.Setloopatomic or RegexNodeKind.Setlazy when nextNode.Kind == currentNode.Kind && currentNode.Str == nextNode.Str:
						...
						if (currentNode.N != Int32.MaxValue)
						{
							currentNode.N = nextNode.N == Int32.MaxValue ? Int32.MaxValue : currentNode.N + nextNode.N;
						}
						next++;
						continue;
					...
					case RegexNodeKind.Set when nextNode.Kind is RegexNodeKind.Setloop or RegexNodeKind.Setloopatomic or RegexNodeKind.Setlazy && currentNode.Str == nextNode.Str:
						if (CanCombineCounts(1, 1, nextNode.M, nextNode.N))
						{
							...
							currentNode.N = nextNode.N == Int32.MaxValue ? Int32.MaxValue : nextNode.N + 1;
							next++;
							continue;
						}
						break;
	...
	private void FindAndMakeLoopsAtomic()
	{
		...
		for (var i = 0; i < childCount - 1; i++)
		{
			...
			static void ProcessNode(RegexNode node, RegexNode subsequent)
			{
				...
				switch (node.Kind)
				{
					case RegexNodeKind.Oneloop or RegexNodeKind.Notoneloop or RegexNodeKind.Setloop when CanBeMadeAtomic(node, subsequent, true, false):
					...
					case RegexNodeKind.Onelazy or RegexNodeKind.Notonelazy or RegexNodeKind.Setlazy when CanBeMadeAtomic(node, subsequent, false, true):
					...
					case RegexNodeKind.Loop when CanBeMadeAtomic(node, subsequent, true, false):
					case RegexNodeKind.Lazyloop when CanBeMadeAtomic(node, subsequent, false, true):
					...
					case RegexNodeKind.Loop:
					{
						...
						if (node.N == 1 || CanBeMadeAtomic(child, child, false, false))
						{
					...
					case RegexNodeKind.Lazyloop:
					{
						...
						if (node.N == 1 || CanBeMadeAtomic(child, child, false, true))
						{
							ProcessNode(child, subsequent);
						}
					}
					...
					{
						var alternateBranches = node.ChildCount();

						for (var b = node.Kind == RegexNodeKind.ExpressionConditional ? 1 : 0; b < alternateBranches; b++)
						{
	...
	private RegexNode ReduceLookaround()
	{
		...
		if (Kind is RegexNodeKind.NegativeLookaround && ContainsKind(Child(0), [ RegexNodeKind.Backreference, RegexNodeKind.BackreferenceConditional ]) is false)
		{
			...
			{
				var node = parent.Child(nodeIndex);
			...
			switch (literal.Kind)
			{
				...
				case RegexNodeKind.Set when loopNode.Kind is RegexNodeKind.Setloop && RegexCharClass.IsSubsetOf(literal.Str!, loopNode.Str!): // e.g. \d+[0-9]\s
					...
					return
						...
						CanBeMadeAtomic(loopNode, afterLiteral, false, false);
	...
	private static bool CanBeMadeAtomic(RegexNode node, RegexNode subsequent, bool iterateNullableSubsequent, bool allowLazy)
	{
		...
		while (true)
		{
			...
			var childCount = subsequent.ChildCount();
			...
			switch (subsequent.Kind)
			{
				...
				case RegexNodeKind.ExpressionConditional when childCount is 3: // condition, yes, and no branch
					for (var i = 0; i < childCount; i++)
					{
						if (!CanBeMadeAtomic(node, subsequent.Child(i), iterateNullableSubsequent, false))
						{
							return false;
						}
					}
			...
			var nextSubsequent = FindNextNodeInSequence(subsequent, out var reachedEnd);
		...
		while (true)
		{
			...
			switch (parent?.Kind)
			{
				...
				case RegexNodeKind.Concatenate:
					...
					var currentIndex = peers.IndexOf(node);
	...
	{
		return IsKnownPrecededOrSucceededByWordChar(false);
	}
	...
	{
		return IsKnownPrecededOrSucceededByWordChar(true);
	}
	...
	private bool IsKnownPrecededOrSucceededByWordChar(bool succeeded)
	{
		...
		while (true)
		{
			...
			switch (parent.Kind)
			{
				...
				case RegexNodeKind.Concatenate:
					var peers = (List<RegexNode>) parent.Children!;
					var index = peers.IndexOf(node) + (succeeded ? 1 : -1);
					...
					if ((uint) index < (uint) peers.Count)
					{
						...
						return
							RegexPrefixAnalyzer.FindFirstOrLastCharClass(peers[index], succeeded) is string set &&
	...
	public int ComputeMinLength()
	{
		...
		switch (Kind)
		{
			...
			case RegexNodeKind.Loop:
				...
				return (int) Math.Min(Int32.MaxValue - 1, (long) M * Child(0).ComputeMinLength());
				...
				// The minimum required length for any of the alternation's branches.
			{
				...
				var min = Child(0).ComputeMinLength();

				for (var i = 1; i < childCount && min > 0; i++)
				{
				...
				// The sum of all of the concatenation's children.
			{
				...
				return (int) Math.Min(Int32.MaxValue - 1, sum);
			}
	...
	public int? ComputeMaxLength()
	{
		...
		switch (Kind)
		{
			...
			case RegexNodeKind.Notonelazy or RegexNodeKind.Notoneloop or RegexNodeKind.Notoneloopatomic or
				...
				return N == Int32.MaxValue ? null : N;
			...
			case RegexNodeKind.Loop or RegexNodeKind.Lazyloop:
				if (N != Int32.MaxValue)
				{
					...
					{
						var maxLength = (long) N * childMaxLength;
			...
			{
				var i = Kind == RegexNodeKind.BackreferenceConditional ? 0 : 1;
	...
	public bool TryGetOrdinalCaseInsensitiveString(int childIndex, int exclusiveChildBound, out int nodesConsumed, [NotNullWhen(true)] out string? caseInsensitiveString, bool consumeZeroWidthNodes = false, bool forPrefixAnalysis = false)
	{
		...
		for (; i < exclusiveChildBound; i++)
		{
			...
			else if (child.Kind is RegexNodeKind.Concatenate)
			{
				...
				if (!StackHelper.TryEnsureSufficientExecutionStack() ||
				    !child.TryGetOrdinalCaseInsensitiveString(0, child.ChildCount(), out var innerNodesConsumed, out var innerStr, consumeZeroWidthNodes, forPrefixAnalysis))
				{
					break;
				}
	...
	{
		return Children as RegexNode ?? ((List<RegexNode>) Children!)[i];
	}
	...
	private string Describe()
	{
		...
		switch (Kind)
		{
			...
			case RegexNodeKind.Lazyloop:
				sb.Append(
					M == 0 && N == Int32.MaxValue ? "*" :
					M == 0 && N == 1 ? "?" :
					M == 1 && N == Int32.MaxValue ? "+" :
					N == Int32.MaxValue ? $"{{{M}, *}}" :
					N == M ? $"{{{M}}}" :
					$"{{{M}, {N}}}");
				break;
